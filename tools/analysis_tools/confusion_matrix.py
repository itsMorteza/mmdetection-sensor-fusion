...
def calculate_confusion_matrix(dataset,
    ...
    for idx, per_img_res in enumerate(results):
        ...
        labels = ann['labels']
        fileindex=dataset[idx]['img_metas'][0].data['ori_filename'].split('.')[0]
        analyze_per_img_dets(confusion_matrix, gt_bboxes, labels, res_bboxes,
                             score_thr, tp_iou_thr, nms_iou_thr,fileindex,MAX_FP)
        prog_bar.update()
...
def analyze_per_img_dets(confusion_matrix,
    ...
    gt_labels=gt_labels[indi]
    gt_bboxes=gt_bboxes[indi]
    lines=[0] * len(gt_bboxes)
    ...
    with open('resultsCONFP/Fusion_cars/'+f'{fileindex}'+'.txt', 'w') as g:
        for line in lines:
           g.write(str(line))
           g.write('\n')
...
def plot_confusion_matrix(confusion_matrix,
    ...
    fig, ax = plt.subplots(
        figsize=(10 * num_classes, 10 * num_classes  ), dpi=300)

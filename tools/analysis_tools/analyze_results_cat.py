...
class ResultVisualizer:
    ...
    def evaluate_and_show(self,
        ...
        for i, (result, ) in enumerate(zip(results)):
            ...
            for j in range(len(data_info['ann_info']['labels'])):
                  if(data_info['ann_info']['labels'][j]==1):
                      bbx.append(data_info['ann_info']['bboxes'][j])
                      lbl.append(data_info['ann_info']['labels'][j])                      
                      
            for k in result:
               print(i,k)
            mAP = eval_fn(result, dict(
            bboxes=bbx, 
            labels=lbl))
            #print(mAP)
            _mAPs[i] = mAP
